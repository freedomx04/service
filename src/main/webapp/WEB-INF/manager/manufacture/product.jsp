...
<script class='template ManufactureProduct' type='text/x-jquery-tmpl'>
	<div class='row'>
		<div class='col-lg-12'>
		...
		<div class='col-lg-12'>
			<div class='module-list'>
				...
				<div class='list-table' style='font-size: 12px;'>
					<table class='table table-hover hm-table' id='manufacture-product-table'>
						<tr><td>
...
<script class='template ManufactureProductDialog' type='text/x-jquery-tmpl'>
      		...
      		<div class="modal-body" style='max-height: 560px; overflow: auto;'>
       			<form class="form-horizontal" role="form">
					...
					<div class="form-group">
  					...
  					<div class="form-group">
   						<label for="clazz" class="col-sm-3 control-label"><i class="form-required">*</i>分类:</label>
    					<div class="col-sm-8">
      						<select class="form-control" id="clazz" name="clazz">
						        <option value="">请选择</option>
								{{~ $k.constant.clazz: clazz}}
							    <option value="{{=clazz.value}}">{{=clazz.name}}</option>
								{{~}}
						    </select>
    					</div>
      				</div>
  					...
  					<div class="form-group">
   						<label for="subClazz" class="col-sm-3 control-label"><i class="form-required">*</i>子类:</label>
    					<div class="col-sm-8">
      						<select class="form-control" id="subCLazz" name="subClazz">
						        <option value="">请选择</option>
						    </select>
    					</div>
      				</div>
  					...
  					<div class="form-group">
   						<label for="property" class="col-sm-3 control-label"><i class="form-required">*</i>厂商性质:</label>
    					<div class="col-sm-8">
      						<select class="form-control" id="property" name="property">
						        <option value="">请选择</option>
								{{~ $k.constant.manufactureProperty: property}}
...
<script class='template ManufactureProductList' type='text/x-jquery-tmpl'>
	<thead>
		<tr>	
	...
	<tbody>
		...
		<tr class='table-tr' id="{{=product.id}}">
			...
			<td>{{=$k.constant.clazz.where('o.value=="'+product.clazz+'"')[0].name}} - {{=$k.constant.clazz.where('o.value=="'+product.clazz+'"')[0].subClazz.where('o.value=="' + product.subClazz + '"')[0].name}}</td>	
			<td>{{=product.price}}</td>
			<td>{{=product.location}}</td>
			<td>{{=product.origin}}</td>
			<td>{{=$k.constant.manufactureProperty[product.property].name}}</td>
		</tr>
		{{~}}
	</tbody>
</script>
	...
	Klass.prototype = Object.create(Object.prototype, {
			...
			value: function($tar) {
				...
				.on('click', '.btn-product-delete', function() {
					...
					.on("click", ".btn-confirm", function() {
						$.ajax({
							url: 'manufacture/product/delete',
							type: 'POST',
							data: {
		...
		draw: {
			value: function(companyId, products) {
				...
				self.$tar.find('#manufacture-product-table').html(
					self.$tempListTar.doT({
						data: products
					})
				);
				self.table = $k.util.dataTable(self.$tar.find('#manufacture-product-table'), {
					'order': [[ 1, 'asc' ]]
				}); 
		...
		load: {
			value: function() {
				...
				$.ajax({
					url: 'manufacture/product/listByCompanyId',
					type: 'POST',
					data: {
